...
class HangmanEnv(ta.Env):
    # Class-level cache: computed once, shared across all instances
    _word_list_cache: Dict[Tuple[bool], List[str]] = {}

    def __init__(self, hardcore: Optional[bool] = False, **kwargs):
        """
        Args:
        ...
        cache_key = (bool(hardcore),)
        if cache_key not in HangmanEnv._word_list_cache:
            self.word_list = words.words("en") if hardcore else words.words("en-basic")
            HangmanEnv._word_list_cache[cache_key] = self.word_list
        else:
            self.word_list = HangmanEnv._word_list_cache[cache_key]
        ...
        if not match:
            ...
            if len(letter) > 1: # Player guessed full word
                if letter == self.state.game_state["target_word"].upper():
                    ...
                    self.state.add_observation(message=f"Your guess of '{letter}' is correct! You win!", observation_type=ta.ObservationType.GAME_MESSAGE)
