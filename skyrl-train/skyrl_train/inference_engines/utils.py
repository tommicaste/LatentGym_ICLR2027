...
def get_phantom_token_logit_bias(tokenizer) -> Optional[Dict[int, float]]:
    """
    Get a logit_bias dict that bans phantom token IDs from sampling.

    Some models have embedding matrices padded beyond the real vocabulary for GPU alignment. 
    This returns {phantom_tid: -100.0} so they are effectively banned from sampling via vLLM's logit_bias parameter.

    Returns None if no phantom tokens exist.
    """
    from transformers import AutoConfig
    num_real_tokens = len(tokenizer)
    if hasattr(tokenizer, 'name_or_path') and tokenizer.name_or_path:
        try:
            ...
            if num_embedding > num_real_tokens:
                return {tid: -100.0 for tid in range(num_real_tokens, num_embedding)}
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(
                f"Failed to load model config for OOV token detection: {e}. "
                "OOV protection will be disabled — phantom tokens may cause crashes during RL training."
            )
    return None
