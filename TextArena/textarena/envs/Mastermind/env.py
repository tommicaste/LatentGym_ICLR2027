...
class MastermindEnv(ta.Env):
    def __init__(self, code_length: Optional[int] = 4, num_numbers: Optional[int] = 6, max_turns: Optional[int] = 20, duplicate_numbers: Optional[bool] = False, turn_penalty: float = 0.0):
        """
        Args:
            ...
            turn_penalty (float): per-turn penalty subtracted from win reward
        """
        super().__init__()
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if black_pegs == self.state.game_state["code_length"]:
            ...
            self.state.set_outcome(reward=win_reward, reason=f"You have cracked the code, solving {black_pegs} out of {self.state.game_state['code_length']} pegs correctly")
