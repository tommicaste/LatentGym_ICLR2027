...
class BaseVLLMInferenceEngine(InferenceEngineInterface):
    ...
    def set_phantom_logit_bias(self, logit_bias: Dict):
        """Store logit_bias on the engine to ban phantom tokens from sampling. Called once at init time."""
        self._phantom_logit_bias = logit_bias
    ...
    def _preprocess_prompts(self, input_batch: InferenceEngineInput):
        ...
        if self._phantom_logit_bias is not None and not sampling_params.logit_bias:
            sampling_params.logit_bias = self._phantom_logit_bias
