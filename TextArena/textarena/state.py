...
class SinglePlayerState(ta.State):
    def __init__(self, num_players: int, seed: Optional[int]=None, max_turns: Optional[int]=None, error_allowance: Optional[int]=None):
        """
        ...
        Args:
            ...
            error_allowance (Optional[int]): Kept for backwards compatibility, not used.
    ...
    def step(self):
        # Turn counting fix: always increment turn, valid or not
        self.turn += 1
    ...
    def set_invalid_move(self, reason: Optional[str], reward: float=-1.0):
        # Turn counting fix: always just add observation, never terminate
        self.made_invalid_move = True
        self.add_observation(message=f"You attempted an invalid move. Reason: {reason}", observation_type=ta.ObservationType.GAME_ADMIN)
...
class TwoPlayerState(ta.State):
    ...
    def set_invalid_move(self, reason: str):
        if self.error_allowance > self.error_count:
            ...
            self.made_invalid_move = True
            self.add_observation(to_id=self.current_player_id, message=f"Player {self.current_player_id} attempted an invalid move. Reason: {reason}", observation_type=ta.ObservationType.GAME_ADMIN)
        else:
