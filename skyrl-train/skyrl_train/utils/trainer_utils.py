...
from skyrl_train.generators.utils import (
    ...
    get_multi_episode_metrics_from_generator_output,
)
...
def calculate_multi_episode_metrics(
    concat_generator_outputs: GeneratorOutput,
    concat_data_sources: List[str],
) -> Dict[str, float]:
    """Calculate multi-episode metrics per data source.

    Needs the following fields for each entry in generator_output["metadatas"]:
        "episode_rewards": List[float] - list of rewards per episode
        "total_episodes": int - total number of episodes in the environment
        "turns_per_episode": List[int] - list of number of turns per episode
    """

    def is_multi_episode_env(metadata) -> bool:
        if "episode_rewards" not in metadata:
            return False
        else:
            return len(metadata["episode_rewards"]) > 1

    concat_is_multi_episode = [is_multi_episode_env(metadata) for metadata in concat_generator_outputs["metadatas"]]
    if not any(concat_is_multi_episode):
        return {}
    ...
    for data_source, indices in data_source_indices.items():
        ...
        multi_episode_metrics = get_multi_episode_metrics_from_generator_output(subset_generator_output)

        sanitized_data_source = sanitize_data_source(data_source)
        for metric_name, metric_value in multi_episode_metrics.items():
            eval_metrics[f"eval/{sanitized_data_source}/multi_episode/{metric_name}"] = metric_value
