...
from skyrl_train.inference_engines.utils import get_sampling_params_for_backend, get_phantom_token_logit_bias
...
class RayPPOTrainer:
    def __init__(
        ...
        # Ban phantom token IDs on each engine to prevent OOV sampling errors
        phantom_logit_bias = get_phantom_token_logit_bias(self.tokenizer)
        if phantom_logit_bias is not None:
            phantom_ids = sorted(phantom_logit_bias.keys())
            logger.info(
                f"Banning {len(phantom_logit_bias)} phantom token IDs "
                f"({phantom_ids[0]}..{phantom_ids[-1]}) via logit_bias to prevent OOV errors."
            )
            asyncio.run(
                self.inference_engine_client._run_on_all_engines(
                    "set_phantom_logit_bias", phantom_logit_bias
                )
            )
    ...
    def convert_to_training_input(self, generator_output: GeneratorOutput, uids: List[str]) -> TrainingInputBatch:
        ...
        # Pass through per-trajectory metadatas (episode_rewards, episode_end_token_indices, etc.)
        if "metadatas" in generator_output and generator_output["metadatas"] is not None:
            training_input.metadata["metadatas"] = generator_output["metadatas"]
    ...
    def compute_advantages_and_returns(self, data: TrainingInputBatch) -> TrainingInputBatch:
        ...
        advantages, returns = ppo_utils.compute_advantages_and_returns(
            ...
            metadatas=data.metadata.get("metadatas", None),
        )
