...
class WordleEnv(ta.Env):
    # Class-level caches: computed once, shared across all instances
    _word_list_cache: Dict[Tuple[int, bool], List[str]] = {}
    _dictionary_cache: Optional[EnglishDictionary] = None

    def __init__(self, word_length: int = 5, num_guesses: int = 6, hardcore: Optional[bool] = False, turn_penalty: float = 0.0):
        ...
        cache_key = (word_length, bool(hardcore))
        if cache_key not in WordleEnv._word_list_cache:
            self._load_word_list(hardcore=hardcore)
            WordleEnv._word_list_cache[cache_key] = self.word_list
        else:
            self.word_list = WordleEnv._word_list_cache[cache_key]

        if WordleEnv._dictionary_cache is None:
            WordleEnv._dictionary_cache = EnglishDictionary(keep_proper_nouns=False, include_nltk=True)
        self.dictionary = WordleEnv._dictionary_cache
    ...
    def reset(self, num_players: int = 1, seed: Optional[int] = None):
        # Turn counting fix: pass num_guesses as max_turns so state tracks it
        self.state = ta.SinglePlayerState(num_players=num_players, seed=seed, max_turns=self.num_guesses)
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        # Turn counting fix: invalid moves count as attempts
        remaining = self.num_guesses - self.state.turn - 1
        if match is None:
            self.state.set_invalid_move(reason=f"Attempts must be in squared brackets. You have {remaining} guesses left.")
            return self._check_turn_limit_and_step()

        word = match.group(1).lower()
        if len(word) != self.state.game_state["word_length"]:
            self.state.set_invalid_move(reason=f"Your word must be exactly {self.state.game_state['word_length']} letters. You have {remaining} guesses left.")
            return self._check_turn_limit_and_step()
        ...
        if word in previous_words:
            self.state.set_invalid_move(reason=f"You have already guessed '{word}' before in this episode. You have {remaining} guesses left.")
            return self._check_turn_limit_and_step()

        if not self._check_word(word):
            self.state.set_invalid_move(reason=f"'{word}' is not an English word. You have {remaining} guesses left.")
            return self._check_turn_limit_and_step()
        ...
        if all(f == "G" for f in feedback):
            turns_used = self.state.turn + 1  # state.turn is 0-indexed, counts all moves (including invalid)
            win_reward = 1.0 - self.turn_penalty * (turns_used - 1)
            self.state.set_outcome(reward=win_reward, reason=f"Congratulations! You guessed the word correctly!")
        else:
            self.state.add_observation(message=f"You submitted [{word}].\nFeedback:\n{self._render_player_view(player_id)}\nYou have {remaining} guesses left.", observation_type=ta.ObservationType.GAME_MESSAGE)

        return self._check_turn_limit_and_step()

    def _check_turn_limit_and_step(self) -> Tuple[bool, ta.Info]:
        # Turn counting fix: check turn limit using state.turn (counts all moves), reward based on last valid guess
        done, info = self.state.step()
        if self.state.turn >= self.num_guesses and not self.state.done:
            pct = self._get_percentage_completion()
            self.state.set_outcome(
                reward=pct,
                reason=f"Turn limit reached. Best guess matched {round(pct * 100)}%. The secret word was: **{self.state.game_state['secret_word']}**."
            )
            return (True, info)
        return (done, info)
