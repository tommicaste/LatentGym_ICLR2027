...
from skyrl_train.generators.base import GeneratorInput, GeneratorInterface, GeneratorOutput, TrajectoryID
...
class SkyRLGymGenerator(GeneratorInterface):
    ...
    async def agent_loop(
        ...
        # Track episode boundary token positions (response-relative indices where each episode ends)
        episode_end_token_indices: List[int] = []
        prev_num_episodes_completed: int = 0
        ...
        while not done:
            ...
            # Detect episode boundaries by watching the episode_rewards list grow in env metadata
            step_metadata = env_step_output.get("metadata", {})
            num_episodes_completed = len(step_metadata.get("episode_rewards", []))
            if num_episodes_completed > prev_num_episodes_completed:
                # An episode just ended; record the response-relative token index
                # response_end_idx is absolute; will be converted to response-relative later
                if per_step_rewards[-1][1] is not None:
                    episode_end_token_indices.append(per_step_rewards[-1][1])
                prev_num_episodes_completed = num_episodes_completed

            if len(input_ids) > max_input_length:
                stop_reason = "length"
                break

        # Record metadata before closing env
        ...
        if retokenize_chat_history:
            ...
            # Convert episode boundary indices to response-relative
            episode_end_token_indices = [idx - initial_prompt_length for idx in episode_end_token_indices]
        ...
        # Store episode boundary token positions in metadata for multi-episode advantage estimators
        metadata["episode_end_token_indices"] = episode_end_token_indices
    ...
    async def generate(self, input_batch: GeneratorInput) -> GeneratorOutput:
        ...
        metadatas = [output.metadata for output in all_outputs]
