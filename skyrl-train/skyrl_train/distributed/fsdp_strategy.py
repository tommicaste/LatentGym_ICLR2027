...
class FSDPStrategy(DistributedStrategy):
    ...
    def load_checkpoint(
        ...
        with io.local_read_dir(ckpt_dir) as read_dir:
            ...
            # Sequential per-rank load + mmap avoids concurrent host-RAM spike on resume.
            optimizer_state_dict = {}
            model_state_dict = None
            extra_state_dict = None
            for r in range(world_size):
                if rank == r:
                    model_state_dict = torch.load(model_path, map_location="cpu", weights_only=False, mmap=True)
                    extra_state_dict = torch.load(extra_path, map_location="cpu", weights_only=False)
                    if optim_exists and load_optimizer_states:
                        optimizer_state_dict = torch.load(optim_path, map_location="cpu", weights_only=False, mmap=True)
                dist.barrier()
